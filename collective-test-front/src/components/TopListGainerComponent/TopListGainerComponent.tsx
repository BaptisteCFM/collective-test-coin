...
import { sortByGain } from '../../lib/CoinsInfos';
import { Coin } from '../../pages/CoinsPage/CoinsPage'
import './TopListGainerComponent.css'

interface DataTopList {
    data: Array<Coin>;
}

const TopListGainerComponent = ({data} : DataTopList) => {
    const [tabData, setTabData] = useState<Array<Coin>>([]);
    useEffect(() => {
        setTabData(sortByGain(data));
    }, [data])

    return (
        <div className='top-coin'>
            <h1><span style={{fontSize: 30}}>🤑</span> Biggest Gainers</h1>
            {
...
export default TopListGainerComponent